...
view(mtcars)
stripchart(disp)
#In stripchart, we cannot identify the number of observations having same value 
#Hence we use stack to plot obs with same values one over the other
#to get the number of such obs having same values 
...
#To remove character part 
gdp_gather$Year <- gsub("[a-zA-Z]", "", gdp_gather$Year)
#Year is still character type even after the removal of "Y"
gdp_gather$Year <- as.integer(gdp_gather$Year)
View(gdp_gather)
glimpse(gdp_gather)
#To remove digit part 
#attach(mtcars)
#mtcars_data <- mtcars
#view(mtcars_data)
