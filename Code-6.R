...
#LDA can be used on datasets which has non-linear separation 
#but it doesn't work well in non-linear separable cases 
...
?apply
#In Second parameter, 1 implies row-wise mean 
#We want column-wise mean, hence 2
...
#LD1 > LD2 : Using LD1, we can clearly discriminate the data.
...
#Define data to plot
#cbind function to gather the data
#x is the linear discriminant values 
...
#To check if the relationship is linear or non-linear 
...
#Both are normally distributed data 
#Theoretical - theoretical concept of normal distribution 
...
#correlation tests.
#Since both mpg and wt are normally distributed, use Pearson correlation. 
