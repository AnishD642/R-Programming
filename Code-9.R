...
#previous period. The naive method seems to work better with data reported on 
...
#forecasting. As with the AIC, the AICc (corrected AIC) should be minimized.
...
#in the forecast; therefore, we want the measure to be sensitive to large errors.
contributions <- c(5,6,3,4,2,0,0,5,7,5,2,1,2,3,2,4,5,6,5,2,5,8,5,5,0,1,4,5,2,2,3,5,6,8,7,6,5,8,5,7,6,2,1)
contributions.ts <- ts(contributions, frequency = 4, start = c(2010,2))
...
#the mean. The ACF plot shows the first spike is outside the blue lines, 
